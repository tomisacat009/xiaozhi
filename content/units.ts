...
export const knowledgeUnits: KnowledgeUnitMeta[] = [
  ...
  },
  {
    ...
    "status": "available",
    "learningGoals": [
      "能从重复乘法和反向求指数理解指数、对数的含义",
      "能判断底数变化对单调性、图像和不等式方向的影响",
      "能用坐标交换解释反函数对称、定义域和值域的关系",
      "能主动检查底数、真数和渐近线等常见限制"
    ],
    "coreTakeaways": [
      "指数给出次数求结果，对数给出结果反求次数",
      "底数必须大于 0 且不等于 1，真数必须大于 0",
      "互为反函数会交换输入输出、定义域和值域，并关于 y = x 对称",
      "底数小于 1 时函数递减，处理不等式必须注意方向反转"
    ],
    "keywords": [
      "指数函数与对数函数对比",
      "functions"
  ...
  },
  {
    ...
    "status": "available",
    "learningGoals": [
      "能区分位置、位移、速度、速率和加速度，并正确处理方向与正负号",
      "能从 x-t、v-t、a-t 图像的斜率、面积和曲率读出运动信息",
      "能解释相等时间内位移及其逐差规律，理解 1:3:5 等常见结论",
      "能从同一运动过程推导并选择匀变速运动的常用公式",
      "能分析加速、减速、停止、反向和退化为匀速等典型情形"
    ],
    "coreTakeaways": [
      "匀变速的本质是任意相等时间内速度变化量相等",
      "x-t 图斜率看速度，v-t 图斜率看加速度、面积看位移，a-t 图面积看速度变化",
      "相邻相等时间内的位移差恒为 a(Δt)²",
      "判断加速或减速要比较速度和加速度方向，不能只看加速度正负",
      "所有常用公式都来自速度线性变化和位移累积，不应孤立死记"
    ],
    "keywords": [
      "匀变速直线运动与图像",
      "等时位移",
